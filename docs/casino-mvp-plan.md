...
**Status:** Spec ✅ (`9c2eaf1`) · R2-1 ✅ (`20a1377` — DeskGame core, sim-v3 tables, ~40 assertions) · R2-2 ✅ (the Desk scene replaces the seven rooms: wallet w/ debt-red, earnings unlock bar, paycheck envelope, catalog + upgrade rows, basic instant-play path; Hud.Casino.cs deleted, single AppPhase.Desk) · R2-3 ✅ (the MAT: per-spot foils on a modal card overlay — up to 9 spots laid out per card, Caz symbol sprites under the foil, printed odds+trap+hardness panel on every card, brush power = Size+Coin upgrades vs Hardness, live match-status line ("2x BELL — one more"), committed-trap ticker, BAIL/CASH OUT resolves with only the revealed mask, leaving = bailing, NEXT-card chaining, card-level-up + unlock celebrations. Trash box cut — no penalty for holding cards, so it'd be pure clutter) · R2-4 ✅ (9-node prestige tree w/ prereqs — Tin Coin / Better Shoes / Sticky Thumb / Card Whisperer roots → Fat Paydays / Robot Arm / Haggler / PP Magnet / Better Lawyer — all effects wired into the core; prestige modal w/ two-tap confirm + ceremony; the ringing phone w/ comedically-printed loan terms; Robot Arm rips the whole pile with one aggregate report) · **R2-5 next** (juice: card physics, scratch audio, debt-red panic, envelope tear-feel) → R2-6 ship prep.
